...
eval (PClass c) = Interp.forwardLink (Interp.boundNames [c]) >> Interp.declareClass c
...
eval (PMethodCall name args) = do
  ...
  case M.lookup name (Interp.env ctx) of
    Just md -> Interp.evalActuals args >>= \actuals -> void $ Interp.runMethod (Interp.listLitAppend (Interp.ListLit [Interp.ObjInstance 0 "" M.empty []]) actuals) md
