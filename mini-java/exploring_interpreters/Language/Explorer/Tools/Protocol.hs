...
type MetaHandler p m c o = Value -> Ex.Explorer p m c o -> m Value
type EIP p m c o = RWST (String -> Either String p, MetaHandler p m c o) S.ByteString (Ex.Explorer p m c o) m

class ExplorerPostValue p c o where
    postExecute :: Ex.Explorer p m c o -> Ex.Explorer p m c o -> o -> Maybe Value
    postExecute = \_ _ _ -> Just Null
    postJump :: Ex.Explorer p m c o -> Ex.Explorer p m c o -> Maybe Value
    postJump = \_ _ -> Just Null
    postRevert :: Ex.Explorer p m c o -> Ex.Explorer p m c o -> [Ex.Ref] -> Maybe Value
    postRevert = \ _ _ _ -> Just Null
...
instance ToJSON ExecuteResult where
    toJSON (ExecuteResult source target out post) = object ["source" .= source , "target" .= target,  "output" .= out, "post" .= post]
...
execute v = case fromJSON v :: Result ExecuteParams of
    ...
    (Success v') -> do
        parser <- lift ask
        let pl = fst parser $ program (v' :: ExecuteParams)
        case pl of
            Right prog -> do
                ex <- lift get
                ...
                return $ toJSON $ ExecuteResult 
                    ...
                    , target = Ex.currRef ex'
                    , output = toJSON output
                    , post = postExecute ex ex' output }
            Left err -> throwE ErrorMessage 
                { code = programParseErrorCode
                , message = "Supplied program is invalid"
                , error_data = Just $ toJSON err }
...
getTrace Nothing = do
    liftIO $ putStrLn "Running gettrace!"
    ex <- lift get
    let trace = Ex.getTrace ex
    let res = toJSON $ map (\(s, (p, o), t) -> Edge { source = fst s, target = fst t, label = EdgeLabel { program = toJSON p, mval = toJSON o} }) trace
    liftIO $ print res
    liftIO $ putStrLn "YES!"
    return res
...
metaCommand :: Value -> ExceptT ErrorMessage (EIP p IO c o) Value
metaCommand val = do
    meta <- lift ask
    ex <- lift get
    liftIO $ snd meta val ex
...
methodDispatch "meta" mval = ensureParameter mval >>= metaCommand
...
serve :: (Eq o, Monoid o, ToJSON o, Eq p, ToJSON p, ToJSON c, ExplorerPostValue p c o) 
    => String -> Ex.Explorer p IO c o -> (String -> Either String p) -> MetaHandler p IO c o -> IO ()
serve port ex parser meta_handler = withSocketsDo $ do
    addr <- resolve port
  ...
  where
    ...
    loop sock = forever $ do
        ...
        forkFinally (talk ex parser meta_handler conn) (\_ -> close conn)
    talk ex parser meta_handler conn = do
        putStrLn "Hello receiving"
        msg <- recv conn 1024
        unless (S.null msg) $ do
            ex' <- acceptCommand ex parser meta_handler conn msg
            talk ex' parser meta_handler conn

acceptCommand ex parser meta_handler conn command = do
        ...
        (AB.Done rem (val, _)) -> do
            case S.length rem < (fromIntegral val) of
                ...
                False -> do
                    ...
                    putStrLn "-------------------------"
                    out <- runRWST (handleRequest' command) (parser, meta_handler) ex
    ...
    case result of
        Nothing -> if toParse == "" then return ex else acceptCommand ex parser meta_handler conn toParse
        Just (resp, ex', log) -> do
            ...
            if toParse == "" then return ex' else acceptCommand ex' parser meta_handler conn toParse