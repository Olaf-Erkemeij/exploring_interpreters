...
toTree exp = mkTree initialRef
  where mkTree ref = Node (ref, fromJust $ deref exp ref) (map (mkTree . fst) (fromJust $ IntMap.lookup ref (children exp)))
...
executionGraph exp =
  ...
  where
    ...
    edges = concatMap (\(p, (prog, out), c) -> [((p, fromJust $ deref exp p), (prog, out), (c, fromJust $ deref exp c))])
...
fromExport :: Language p m c o
           ...
           -> Explorer p m c o
fromExport exp (curr, nds, edgs) = exp
    { genRef = findMax nds
