...
mkExplorer maxPath maxDegree definterp conf =
  Explorer
    { defInterp = definterp,
      config = conf,
      currRef = initialRef,
...
applyPatchPath e conf (p : ps) =
  lab (execEnv e) p >>= \pr -> case IntMap.lookup pr (cmap e) of
...
generatePatch e c
  ...
  where
    dist = case IntMap.lookup (currRef e) (cmap e) of
      Nothing -> 1
      Just (FullConfig _) -> 1
