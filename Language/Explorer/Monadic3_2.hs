...
module Language.Explorer.Monadic3_2
  ( Explorer,
    mkExplorer,
    mkExplorerNoSharing,
    execute,
        ...
        return $
          e'
            { execEnv = mkGraph (zip remainNodes remainNodes) $ cleanEdges reachNodes (labEdges $ execEnv e'),
              cmap = cmap e V.// [(indexOf r, Nothing) | r <- reachNodes]
            }
  | otherwise = Nothing
  where
...
toTree exp = mkTree initialRef
  where
    graph = execEnv exp
    target (_, r, _) = r
    mkTree r = Node (r, fromJust $ deref exp r) (map (mkTree . target) (out graph r))
...
toExport exp = (currRef exp, getNodeList exp, labEdges $ execEnv exp)
